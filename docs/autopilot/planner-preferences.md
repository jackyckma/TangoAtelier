...
- M6 development (E-04): **extend** `backend/app/engine/melody/development.py` + wire into existing `skeleton.py` motivic_cells / PitchCell paths. Decompose: pure contract (enum + SECTION_TECHNIQUES + MotifPlan) → phrase technique tags → setup/payoff wire → similarity acceptance. No new user-facing flag; teaching fields export on melody bars only.
